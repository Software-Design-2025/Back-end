...
const { transcribe } = require('../helpers/ai-services.helper');
const {
  insertVideo,
  getVideos,
  getPublicVideos,
...
async function download(source) {
  ...
  if (!response.ok || !response.body) {
    throw new Error(
      `Failed to download ${source} - status: ${response.status}`
    );
  }
...
async function createVideos({ width: width, height: height, scenes: scenes }) {
  let promises = [];
  let tempfiles = [];
...
async function createTranscription(video) {
  ...
  await new Promise((resolve, reject) => {
    ffmpeg(video)
      .outputOptions([`-t ${duration}`, '-map 0:a', '-c:a mp3'])
      .output(audioFile)
      .on('end', () => resolve())
      .on('error', (err) => {
...
async function addSubtitles(video) {
  ...
  await new Promise((resolve, reject) => {
    ffmpeg()
      .input(video)
      .videoFilters(
        `subtitles='${subtitleFile.replace(/\\/g, '\\\\').replace(/:/g, '\\:')}'`
      )
...
async function uploadToFirebase(filePath) {
  const fileRef = ref(
    storage,
    `ai-short-video-files/${path.basename(filePath)}`
  );
